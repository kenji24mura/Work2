...
var GIS_SERVER_IP = "172.20.10.27"; //Azure
...
var MapObj = {
  ...
  changeVisibleLayer(layerId, isVisible, callback) {
    ...
    if (ope_mode == 1) {
      ...
      if (layerId == "n@1-1") {
        //				layerId = "n@12-1";
        layerId = "n@32-1"; //20241024
      }
      if (layerId == "n@10-1") {
        //				layerId = "n@13-1";
        layerId = "n@33-1"; //20241024
      }
      if (layerId == "n@14-1") {
        isVisible = false;
      }
  ...
  changeVisibleLayers(layerIds, isVisible, callback) {
    ...
    for (var i = 0; i < layerIds.length; i++) {
      ...
      if (layerIds[i] == "n@1-1") {
        //				layerId = "n@12-1";
        layerIds[i] = "n@32-1"; //20241024
      }
      if (layerIds[i] == "n@10-1") {
        //				layerId = "n@13-1";
        layerIds[i] = "n@33-1"; //20241024
      }
      if (layerIds[i] == "n@14-1") {
        layerIds[i] = false;
      }
      if (layerIds[i] == "n@15-1") {
        isVisible = false;
      }
      if (layerIds[i] == "n@16-1") {
        isVisible = false;
      }
...
/*
    var info={};
    info.wkt = "POLYGON(135.4647699999 34.6691313888, 135.4652088888 34.6693063888, 135.465215 34.6683213888, 135.4650683333 34.6685883333, 135.4649358333 34.6688302777, 135.4647699999 34.6691313888)";
info.attributes = "";
    data.push(info);

info={};
info.wkt = "POLYGON(135.4669511111 34.6702155555, 135.4671852777 34.6703988888, 135.4676672222 34.6708175, 135.4677027777 34.6707336111, 135.4679897222 34.6702319444, 135.4680558333 34.6701166666, 135.4682733333 34.6697380555, 135.4683183333 34.6696597222, 135.4686241666 34.6691286111, 135.46873 34.6689433333, 135.46875 34.6689091666, 135.4690180555 34.6684497222, 135.4691105555 34.6682922222, 135.4693091666 34.6679425, 135.4693491666 34.6678686111, 135.4654811111 34.6678516666, 135.46546 34.6678894444, 135.4652591666 34.6682430555, 135.465215 34.6683213888, 135.4652088888 34.6693063888, 135.4656408333 34.6694697222, 135.4658347222 34.6695463888, 135.4658683333 34.6695591666, 135.4660708333 34.6696619444, 135.4664044444 34.6698508333, 135.4667338888 34.6700577777, 135.4668469444 34.6701394444, 135.4669511111 34.6702155555)";
info.attributes = "";
    data.push(info);
...
info={};
info.wkt = "POLYGON(135.4693491666 34.6678686111, 135.4695033333 34.6676113888, 135.4687505555 34.6673177777, 135.4685283333 34.6672311111, 135.4677469444 34.6669269444, 135.4675127777 34.6668358333, 135.4670883333 34.6666708333, 135.4664875 34.6664363888, 135.4664622222 34.6664786111, 135.4663130555 34.6664202777, 135.4661694444 34.6666702777, 135.4661155555 34.6667627777, 135.4654811111 34.6678516666, 135.4693491666 34.6678686111)";
info.attributes = "";
    data.push(info);
*/
    facade.drawItems(
        ...
        if (click_mode == "getaddr") {
          ...
          facade.getAddressFromPosition(
            ...
            function (ret) {
              ...
              var url =
                "https://" + GIS_SERVER_IP + ":8443/Sample/GetAddrPolygon2?";
              url += "prif=";
              url += prif;
              url += "&city=";
              url += city;
  ...
  videoView(termID, callback) {
    ...
    if (init_terminal_type == "0") {
      ...
      if (init_terminal_type == "1") {
        //2025.01.06
        /*
        url = "VideoView.html?id=" + termID;
        url += "&terminal_type=";
        url += init_terminal_type;

        url2 = "VideoView2.html?id=" + termID;
        url2 += "&terminal_type=";
        url2 += init_terminal_type;
*/
        ...
        for (var i = 0; i < res.data.length; i++) {
          for (var j = 0; j < param.length; j++) {
            ...
            if (res.data[i].car_cd == param[j]["id"]) {
              ...
              //20260606
              attr["備考"] = "車両名:"+res.data[i].sharyo_cd+",車両動態（基本）:"+res.data[i].dotai_kihon+",車両動態（詳細）:"+res.data[i].dotai_shosai;;
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          console.log("ope_mode=" + ope_mode);
          console.log("res.data[i].info_type=" + res.data[i].info_type);
          //if (ope_mode == 1) {
          //	//訓練時
          //} else {
          //	if (res.data[i].info_type != 13) {
